...
export default function Checkout(props){
    ...
    let totalPrice = cart.orders.map(o => o.product.price).reduce((a, b) => a + b, 0)
    ...
    let purchased = () => {
        ...
        .then(res => res.json())
        .then(user => (
            props.history.push('/home', user.id)
        ))
    }
    ...
    return(
        <div>
            ...
            <div class="container">
                <h4>Cart<span><b>{cart.id}</b></span></h4>
                {cart.orders.map(order => (
                    <p>{order.product.name}<span>${order.product.price}</span></p>
                ))}
                <hr/>
